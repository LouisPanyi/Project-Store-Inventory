...
export default function CreateTransaksiForm({ produkList }: { produkList: Produk[] }) {
  ...
  const initialState: StateTransaksi = { message: null, errors: {}, success: false };
  ...
  useEffect(() => {
    if (state.success && state.transaksiId) {
      const trxId = state.transaksiId;

      if (paymentMethod === "qris") {
        // ambil qris_url dari API
        fetch(`/api/transaksi/${trxId}`)
          .then((res) => res.json())
          .then((data) => {
            if (data.qris_url) {
              setQrisUrl(data.qris_url);
              setShowQris(true); // buka popup QRIS
            }
          });
      } else {
      ...
      <form action={formAction} className="space-y-6">
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {produkList
                    .filter(
                      (p) => !rows.some((r, rIdx) => rIdx !== idx && r.produkId === p.produk_id)
                    )
                    .map((p) => (
        ...
        <div className="rounded-lg border bg-white shadow-sm p-6 space-y-3">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            <label className="w-full md:w-1/3 font-medium">Metode Pembayaran</label>
            <select
              name="payment_method"
              value={paymentMethod}
              onChange={(e) => setPaymentMethod(e.target.value)}
              className="w-full md:w-2/3 rounded-md border border-gray-300 py-2 pl-2 text-sm focus:ring-2 focus:ring-indigo-500"
              required
            >
          ...
          {paymentMethod === "cash" && (
            <div className="flex flex-col md:flex-row md:items-center gap-3">
              <label className="w-full md:w-1/3 font-medium">Bayar (cash only)</label>
              <input
                type="number"
                name="pay"
                value={pay}
      ...
      {showQris && qrisUrl && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg space-y-4">
            <h2 className="text-lg font-semibold">Scan QRIS untuk Bayar</h2>
            <Image
              src={qrisUrl}
              alt="QRIS"
              width={256} 
